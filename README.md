...
서버 상태를 `react-query`에 위임하여 개발 생산성을 높이고 **[stale-while-revalidate](https://web.dev/stale-while-revalidate/)** 방식에 따라 개발자가 전략적으로 사용할 수 있습니다. 이에 따라 클라이언트단의 전역 상태에 대한 의존도가 줄어들게 되었습니다.

클라이언트단 전역 상태 관리에는 `Redux`처럼 Flux 방식으로 전역 상태를 단방향 관리할 수 있으면서도, `immer` 등 다른 써드 파티 라이브러리와 통합이 용이한 `Zustand`를 선택하였습니다. 구현 자체도 간단하게 되어있어 번들 용량도 작습니다. :)
...
텍스트 채팅 메시지를 입력할 때 사용자는 가독성을 위해서 줄바꿈을 사용하여 입력하기도 합니다. 채팅 입력 필드도 줄바꿈이 지원이 되어야 합니다. 이를 위해 `react-textarea-autosize`를 사용했습니다.

`react-dropzone`은 Drag and Drop 기능 뿐만 아니라, 파일 크기 및 타입에 따른 처리를 용이하게 할 수 있는 API를 제공하고 있습니다. 채팅방의 사진 업로드 기능은 드래그 앤 드롭을 사용하지 않지만, 파일 크기와 타입에 따른 예외 처리가 필요하여 사용하게 되었습니다.
...
개발 단에서 컴포넌트 단위로 개발하면서도, 이를 문서화하여 정리할 수 있도록 사용하였습니다.

한편, `Figma`와 `Zeplin` 등 디자이너의 협업 툴에서 디자인 토큰을 생성하여 편리하게 통합할 수 있는 `style-dictionary`도 함께 사용하였습니다. 다만 과제의 디자인 명세는 특정 계층이나 규칙을 가지고 디자인 토큰이 정의가 되어 있다보니 용이하게 활용하지는 못했습니다.
