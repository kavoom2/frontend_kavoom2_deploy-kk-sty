...
`CRA`와 `React`는 각각 5, 18버전을 사용하고자 했으나, `Webpack 5`의 Polyfill 이슈와 `Storybook 7`과 `Node 14`와의 호환성 이슈로 각각 4, 17버전을 사용하였습니다. 이에 대한 자세한 내용은 하단의 고려사항 항목에 기술하였습니다.
...
한편, `Figma`와 `Zeplin` 등 디자이너의 협업 툴에서 디자인 토큰을 생성하여 편리하게 통합할 수 있는 `style-dictionary`도 함께 사용하였습니다. 다만 과제의 디자인 명세는 특정 계층이나 규칙을 가지고 디자인 토큰이 정의가 되어 있지 않아 용이하게 활용하지는 못했습니다.
...
#### 1. Webpack Polyfill 이슈와 Node 버전 제약에 따른 초기 개발 환경 구성

초기 개발 환경 설정에서 `react-scripts 5`과 `react/react-dom 18`으로 구성하여 진행하고자 하였습니다. `react-scripts`, `storybook`에서 사용하는 `Webpack 5`에서 더 이상 Nodejs API에 대한 Polyfill을 제공하지 않아 `CRA`, `Storybook` 개발 환경을 실행할 수 없는 이슈가 발생했습니다. Nodejs API에 대한 Polyfill을 `Webpack` 설정에 적용하여 해결할 수 있으나, 추후 다른 개발 도구와 통합하는 과정에서 동일한 이슈가 반복해서 발생할 여지가 있습니다.

한편 `react 18`을 사용하는 개발 환경에서는 `Storybook 7`이상을 사용해야 합니다. 하지만 `Storybook은` v7 부터 `Node 14`를 지원하지 않습니다. (최소 v16 이상이어야 합니다.)

위와 같은 이유로 `react-scripts`를 `4.0.3`, `react/react-dom`을 `17.0.2`으로 다운그레이드하여 진행하였습니다.
